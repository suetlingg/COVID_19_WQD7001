...
ui <- dashboardPage(
  dashboardHeader(title = "Malaysia covid19 "),
  dashboardSidebar(
...
server <- function(input, output) {
  ...
  output$Plot3_1 <- renderPlotly({
    ...
    ggplot(data=data3()) + 
      ...
      labs(title = " total cases new  of malaysia")+
      theme(axis.text.x = element_text(vjust = 0.5),
            plot.title = element_text(hjust = 0.5))
  ...
  output$Plot3 <- renderPlotly({
    ...
    ggplot(data=data2()) + 
      ...
      coord_sf() + 
      scale_fill_continuous(type = "viridis") +
      labs(title = " total new deaths   of malaysia")+
      theme(axis.text.x = element_text(vjust = 0.5),
            plot.title = element_text(hjust = 0.5))
  ...
  # labs(title = "death rates of Malaysia Before and after vaccination")+
  # theme(axis.text.x = element_text(vjust = 0.5),
  #       plot.title = element_text(hjust = 0.5))
  ...
  output$Plotd <- renderPlot({
    ...
    ggplot(data = datad(),aes(x=vaccine_percent,y=icu_covid_rate))+
      ...
      geom_smooth(se=FALSE)+
      labs(title = "vaccine_percent vs icu_covid_rate ")
  ...
  output$Plota4 <- renderPlot({
    ...
    ggplot(data =datad(),aes(x=reorder(state,icu_covid_rate),y=icu_covid_rate))+ 
